...
loaded_adapters = []
# task_name = "arc_easy"
tasks = ["leaderboard_bbh", "arc_challenge", "hellaswag", "arc_easy", "anli_r1", "piqa"]
current_input_id = None
handle_q = None
handle_v = None
top_k = 1
output_file = "logo_result.txt"

adapter_paths = ["lora_adapters_fine_tuned/6_1/amazon_polarity_Is_this_product_review_positive.json",
                 "lora_adapters_fine_tuned/6_1/anli_r3:0.1.0.json",
                 "lora_adapters_fine_tuned/6_1/cos_e_v1.11_aligned_with_common_sense.json",
                 "lora_adapters_fine_tuned/6_1/glue_mnli:2.0.0.json",
                 "lora_adapters_fine_tuned/6_1/sciq_Multiple_Choice.json",
                 "lora_adapters_fine_tuned/6_1/social_i_qa_Generate_answer.json",
                 "lora_adapters_fine_tuned/6_1/super_glue_cb:1.0.2.json",
                 "lora_adapters_fine_tuned/6_1/trivia_qa_rc:1.1.0.json",
                 "lora_adapters_fine_tuned/6_1/wiqa_effect_with_string_answer.json",
                 "lora_adapters_fine_tuned/6_1/yelp_polarity_reviews:0.2.0.json",
                ]
    ...
    for i in adapter_paths:
        results = lm_eval.simple_evaluate(
            ...
            model_args={
                ...
                "peft": i, # Path to your LoRA adapter
                "device": "cuda:0",
            },
        ...
        a_name = os.path.basename(i).replace(".json", "")
        a_name = os.path.basename(i).replace("lora_adapters_fine_tuned/6_1/", "")
        with open(output_file, "a", encoding="utf-8") as f:
            f.write(f"\n{'='*60}\n")
            # f.write(f"Task: {task_name} | Top_K: {controller.top_k} | Batch Size: 1\n")
            f.write(f"Task: {task_name} | Top_K: {a_name} | Batch Size: 1\n")
            f.write(f"Final Score (Acc): {score}\n")
            f.write(f"{'-'*60}\n")
            f.write(table_results)
            f.write(f"\n{'='*60}\n")

        print(f"✅ {task_name} Score: {score} - Saved to {output_file}")    
                
        # # 3. Extract Specific Metrics
        # # Example: Extracting the accuracy score for a specific task
        # if j in results["results"]:
