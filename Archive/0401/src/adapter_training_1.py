...
if __name__ == "__main__":
    ...
    train(
        ...
        adapter_name = "lora",
        wandb_project = "LoGo Adapters_6",
        wandb_run_name = task,
        wandb_watch = "false",  # options: false | gradients | all
        wandb_log_model = "false",  # options: false | true
        resume_from_checkpoint = "./lora_adapters_fine_tuned/6_1/super_glue_cb:1.0.2.json/checkpoint-100",  # either training checkpoint or final adapter
        num_epochs = 200,
        eval_step = 50,
        save_step = 50,
        lora_target_modules = ['layers.31.self_attn.q_proj', 'layers.31.self_attn.v_proj'],
    )
