...
class LoGO_controller:
    def __init__(self, config, scaling, adapters_list, device, dtype):
    ...
    def controller_pre_hook(self, module, args, kwargs):
        ...
        if seq_len > 1:
            ...
            with torch.no_grad():
                ...
                qw = torch.softmax(q_norms.view(-1, self.batch_size), dim=0) # [N, B]
                ...
                vw = torch.softmax(v_norms.view(-1, self.batch_size), dim=0) # [N, B]
                ...
                self.current_weights = (k_masked_weights / (k_masked_weights.sum() + 1e-6)).view(-1, self.batch_size, 1, 1) # [N, B, 1, 1]
    ...
    def apply_adapter_hook(self, module, input, output, mode='q'):
        ...
        with torch.no_grad():
            ...
            x = x.unsqueeze(0)
            self.current_weights = self.current_weights.view(-1, self.batch_size, 1, 1) # [N, B, 1, 1]
            ...
            print(f"\n{'='*20} DEBUG: {mode} {'='*20}")
            print(f"[DEBUG] Input Shape: {list(input[0].shape)}") # input은 tuple이므로 [0] 참조
            print(f"[DEBUG] Deltas Shape: {x.shape}")
            print(f"[DEBUG] Final Delta Shape: {final_delta.shape}")
            print(f"\n{'='*20} DEBUG: {mode} {'='*20}")
...
# 4. 훅 등록
controller = LoGO_controller(config=config, adapters_list=loaded_adapters, scaling=64/32, device = base_model.device, dtype=base_model.dtype)

attn_module = base_model.model.layers[config.target_layer_idx].self_attn
q_proj = attn_module.q_proj
v_proj = attn_module.v_proj

pre_handle = attn_module.register_forward_pre_hook(controller.controller_pre_hook, with_kwargs=True)
q_handle = q_proj.register_forward_hook(partial(controller.apply_adapter_hook, mode='q'))
v_handle = v_proj.register_forward_hook(partial(controller.apply_adapter_hook, mode='v'))


test_input = tokenizer("Hello, how are you?", return_tensors="pt").to("cuda")
base_model.generate(**test_input, max_new_tokens=5)

# for i in [10, 20]:
#     config.top_k=i
#     controller.top_k = config.top_k
...
#         batch_size=config.batch_size,               # Adjust based on your GPU VRAM
...
#     # for t_name, t_metrics in results["results"].items():
#     # score = t_metrics.get("acc,none") or t_metrics.get("acc") or "N/A"
#     # task_metrics = results["results"].get(config.task_name, {})
#     # score = task_metrics.get("acc,none") or task_metrics.get("acc") or "N/A"

#     with open(config.output_file, "a", encoding="utf-8") as f:
#         f.write(f"\n{'='*60}\n")
#         f.write(f"Top_K: {config.top_k} | Batch Size: {config.batch_size}\n")
